...
public class TCPServer {
	...
	public void serverStart() {
		...
		try {
			...
			// InputStreamReader : 문자기반 스트림과 바이트 기반 스트림 연결에 사용되는 스트림 보조스트림
			...
			Date now = new Date(); // 생성된 시간을 기록하고있는 기간 관련객체
			
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
			
			String time = sdf.format(now); // now에 저장된 시간 포맷을 변경
			
			pw.println(time +"[서버 접속성공]");
			...
			try {
				...
				// if문에 {}를 사용 안하면 다음 한줄(세미클론 기준)이 if문 내부 코드가된다
			} catch (IOException e) {
