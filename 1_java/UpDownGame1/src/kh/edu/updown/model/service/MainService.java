...
public class MainService {
		...
		do {
			try {
				...
				if(loginMember == null) { // 로그인이 되어있지 않은 경우
					...
					sel = sc.nextInt(); // ★★ 다음은 nextline 필수
					sc.nextLine();
					System.out.println();
	...
	public void signUp() { 
		...
		if(memberCount >= members.length) {
			System.out.println("회원 정원이 가득차 더이상 가입을 할 수 없습니다.");
		} else { // 회원 가입이 가능한 경우
			...
			while(true) {
				...
				for(int i=0; i<members.length; i++) {
					if(members[i] !=null) { // members 배열 요소가 null이 아닌경우 == 회원정보가있는경우
						// 입력 받은 아아디와 같은 아이디가 members에 존재하는 경우
						if(memberId.equals(members[i].getMemberId() )) {
							System.out.println("중복되는 아이디 입니다. 다시입력해주세요.");
				...
				if(flag) { // 중복 아아디가 아닌경우
					break; // 아이디를 반본적으로 입력 받는 while문 종료
				}
			...
			// 입력 받은 값을 이용해 새로운 Member 객체를 생성하고 member 배열의 요소 중
	...
	public void login() {
		...
		for(int i=0; i<members.length; i++) { //members 배열 요소가 null이 아닌경우 == 회원정보가있는경우
			
			if(members[i] !=null) {
				
				//members 배열 요소 중입력받은 아이디,비밀번호가 일치하는 회원이 있을경우
				if(members[i].getMemberId().equals(inputId) && members[i].getMemberPw().equals(inputPw)) {
					...
					loginMember = members[i]; // loginMember 필드에 일치하는 회원객체의 주소를 얕은복사
											  // 같은 주소를 참초하고있다
				}
					
			}else { // members 배열 요소가 null인 경우 == 회원 정보가 없는경우
				break;
			}
		} // end for
		
		// 아이디,비밀번호를 비교한 후에도 loginMember필드가 null인 경우 == 로그인실패
		
		if(loginMember == null) {
			System.out.println("아이디 또는 비밀번호가 일치하지안습니다");
