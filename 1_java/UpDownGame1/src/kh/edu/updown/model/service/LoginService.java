...
public class LoginService {
	...
	public void startGame(Member loginMember) {
			...
			try {
				...
				while(true) {
					count++;// while문이 반복될 때마다 count를 1증가시킴
					
					System.out.println(count + "번째 입력 : ");
					int input = sc.nextInt();
					sc.nextLine();
					
					if(random == input) { // 입력된값이 발생한 난수와 같다면
						System.out.println("정답!");
						System.out.println("입력 시도 횟수 : "+count);
						
						// 입력시도 횟수가 최초 또는 최고 기록인경우
	...
	public void selectMyInfo(Member loginMember) {
			...
			System.out.println("아아디 : "+loginMember.getMemberId()); 
	...
	public void selectAllMember(Member[] members) {
		...
		for(int i=0; i<members.length; i++) {
			...
			if(members[i] !=null) { // members 배열 요소에 저장된 회원 정보가 있는경우
