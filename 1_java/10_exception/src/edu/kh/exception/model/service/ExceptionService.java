...
public class ExceptionService {
	...
	// 에외 (Exception) : 소스 코드의 수정으로 해결 가능한 오류

	// 예외는 두 종류로 구분됨
	// 1) UnCheked Exception : 선택적으로 예외 처리
	...
	public void ex1() {
		...
		// try { } : 괄호 내부에 예외가 발생할 가능성이있는 코드를 작성한 후 시도

		// catch(예외){ } : try 구문에서 발생할 예외를 잡아내서 처리하여
		...
		/*
		 * try { System.out.println("결과 : "+ input1 / input2); //
		 * java.lang.ArithmeticException: / by zero // 산술적 예외 : 0으로 나눌 수 없습니다.
		 * 
		 * } catch(ArithmeticException e) { // try에서 던져진 예외를 catch문 매개변수로 잡음
		 * 
	...
	public int inputNumber() {
		...
		while (true) {
			try {
				...
				num = sc.nextInt();// java.util.InputMismatchException : 잘못 입력 에외
	...
	public void ex3() {
		...
		try {
			...
			// * NullPointerException : 참조하지 않는 참조변수를 이용해서 코드를 수행할때 발생
			...
			// ** 상위 타입의 예외 클래스[Exception]를 catch문에 작성하면
	...
	public void ex4() {
		...
		try {
			...
			// 어디서 어떤 에러가 발생했는지 정학히 파악하는 용도

		} finally { // 외부 프로그램(DB)과의 연결 끊기 시 주로사용
	...
	public void methodC() throws IOException {
		...
		// 단, UnCheckedException은 선택적으로 예외처리할 수있음
	...
	public void methodD() {
		...
		// - if문과 같은 단순 코드로 해결이 불가능하여 반드시 예외 처리 구문을 작성해야하는 예외
