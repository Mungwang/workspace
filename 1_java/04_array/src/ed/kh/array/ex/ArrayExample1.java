...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		// 1-1. 변수 선언
		int num;
		// stack 영역에 int 자료형을 저장 할 수 있는공간 4byte를 생성(할당)하고
		// 그 긍간에 num 이라는 이름울 부여
		...
		// 그 고간에 arr 이라는 이름을 부여
		// *** 해당 변수는 참조형으로 주소 값(8byte)만을 저장할 수있음
		
		// 2-2. 배열 할당
		arr =new int[3];
		
		// new int[3] 구문으로 생성된 배열의 시작주소를 
		// arr 변수에 대입
		...
		// arr = new int [3];
		// (int[]) (new int[]) -> 같은 자료형 == 연산가능
		
		// heap 영역에 생성된 int[]의 시작주소를 Stack 영엑에 생성된 arr 변수에 대입
		...
		// 2-3. 배열 요소 값 대입
		// arr은 int[] 참조형 변수 이지만
		// arr[0] int 자료형 변수이기 떄문에 정수 값을대입할 수 있다.
		arr[0] = 10;
		arr[1] = 50;
		arr[2] = 10000;
		...
		System.out.println(arr[0]); // arr이 참조하고 있는 배열의 0번 인덱스 값을 얻어
		System.out.println(arr[1]); // arr이 참조하고 있는 배열의 1번 인덱스 값을 얻어
		System.out.println(arr[2]); // arr이 참조하고 있는 배열의 2번 인덱스 값을 얻어
	...
	public void ex2() {
		...
		// 2) heap 영역에 1nt 저료형 4개를 묶음으로 다루는 int[] 할당
		...
		for(int i=0; i<arr.length; i++) {
			...
			System.out.printf("arr[%d] 저장된 값 : %d\n",i,arr[i]);
		}
		// ★★★★(참고지만 알고있는게 좋음)★★★★
		
		// 비어있다 : stack 영역에 선언된 변수에 값이 대입되지않음
	...
	public void ex3() {
		...
		// doublep[] 자료형 참조변수 height을 stack영역에 생성하고
		// heap영역에 새로 생성된 double 5칸 짜리 double[]의 시작 주소를 height대입
		...
		for(int i=0; i<height.length; i++) {
			...
			sum += height[i]; // 배열에 저장된값을 sum에 누적
		}
	...
	public void ex4() {
		...
		//이때, score[i] 값이 max보다 크면 max에 대입
	...
	public void ex5() {
		...
		// char[] arr이 참조하늘 배열 요소에 A,B,C,D,E 대입하기
		for(int i=0; i < arr.length; i++) {
			arr[i] = (char)('A' + i);
			// A== 65 B== 66 c==67
		...
		int[] arr2 = new int[4];
		System.out.println(arr2); //[I@36d64342 == I-int @-구분자 36d64342-해시쿼드 주소가암호와된값
	...
	public void ex7() {
		...
		boolean flag = false; // 검사 전에는없다고 가정
		
		// arr 배열 요소 순차 접근(반복 접근)
		
		for(int i=0; i<arr.length; i++) {
			if(input==arr[i]) { // arr[i] 0~9까지 반복하니까 i를 적는다
	...
	public void ex8() {
		...
		// 입력 받읍 값과 일치하는 값이 있으면 인덱스 번호 출력
		// 없으면 "존재하지 않음"
	...
	public void ex9() {
		...
		// [사용 해야되는 기술, 기능]
		// 1) 배열 검색 2) String.length() : 문자열의 길이
		//				ex) "Hello".length() -> 5
		...
		String input = sc.nextLine(); // 한 줄(띄어쓰기포함) 입력받기
		...
		for(int i=0; i<arr.length; i++) {
			...
			arr[i] = input.charAt(i); // arr[0] = hello world.charAt(0) ==h
