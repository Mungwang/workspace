...
public class ArrayExample2 {
	...
	public void deepCopy() {
		...
		// 1. for문을 이용한 깊은 복사
		int[] copyArr1 = new int[arr.length]; // 5칸짜리 배열생성된다.
		
		for(int i=0; i<arr.length; i++) {
			copyArr1[i] = arr[i];
		}
		// 2. System.arraycopy
		// (원본배열, 원본 복사시작 인덱스,복사배열,복사배열의 삽입 시작 인덱스,복사길이);
	...
	public void createLottoNumer() {
		// 로또 번호 생성기
		
		// 1. 1~45사이에 중복되지 않은 난수 6개 생성
		// 2. 생성된 난수가 오름차순으로 정렬
		...
		// 2) 생선된 배열을 처음부터 끝가지 순차 접근하는 for문 작성
		...
		for(int i=0; i<lotto.length; i++) {
			...
			for(int x=0; x<i; x++) {
				...
				if(random == lotto[x]) {
					...
					//	   i값을 인위적으로 1 감소시켜 7회 반복되는 모양을 만듬
					
					break;
					// 앞쪽에서 중복 데이터를 발견하면 남은 값을 비교할 필요가없음
