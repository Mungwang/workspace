...
public class PolyService {
	...
	public void ex1() {
		...
		// 1) 자식객체가 부모 객체로 변하였기 때문에 자식만의 고유한 필드,메소드를 사용할수 없다.
		
		// 1-1) car (부모 = 부모)
		car.setEngine("v6 6기통엔진");
		car.setFuel("휘발유");
		car.setWheel(4);
		// Car 메소드 모두 사용가능
		...
		tesla.setFuel("전기");
		tesla.setWheel(4);
		tesla.setBatteryCapactiy(1000000);
		// Tesla 메소드 모두 사용 가능
		...
		Car[] arr = new Car[3]; // 부모 타입 참조변수 배열 선언및 할당
								// 각 배열 요소가 Car타입 참조변수
		
		arr[0] = car; // Car 주소 == Car 객체 
		// Car 참조 변수
		
		arr[1]= car2; // Tesla 즈소 == Tesla 객체
		// Car 참조 변수
		...
		//			-> Car 상속 클래스는 모두 getEngine()을 가지고있다 정의
	...
	public void ex2() {
		...
		Car[] arr = { createrCar(1), createrCar(2), createrCar(3) };
	...
	// 전달 받은 Car 또는 자식 객체의 엔진, 연로, 바퀴 개수를 출력하는 ㅔㅁ소드
	// 매개변수에 부모 타입 참조변수를 작성하면 모든 자식 객체를 전달 받을 수있다.
	public void printCar(Car temp) {
		// 매개변수에 작성된 참조형 변수에는 주수가 저장된다.(얕은 복사)
	...
	// 생성된 객체의 주소를 반환
	public Car createrCar(int num) {
		
		Car result = null; // null == 아무것도 참조하고있지 않음
	...
	public void ex3() {
		...
		// ***** 다형성 중 다운 캐스팅 *****
		// - 다운 캐스팅이란?
		// 부모 타입 참조변수가 자식 객체를 참조하는 업캐스팅 상태에서만 진행할 수있는 기술로
		
		// 부모 타입을 자식 타입으로 "강제 형변환"해서 자식 객체의 본래 필드, 메소드를 사용 가능하게 한다.
		
		Car c1 = new Tesla("전기모터","전기",4,50000);
		System.out.println( ((Tesla)c1).getBatteryCapactiy() );
		// 주의!!! "." 연산자가 (Tesla) 형변환 연산자보다 우선 순위가 높음
		...
		System.out.println(t1.getBatteryCapactiy());
	}
	...
	public void ex4() {
		...
		// -> c1이 참조하는 객체는 Tesla인데 Spark 참조변수로 Tesla 참조하려고 해서 문제발생
	...
	public void ex5() {
		...
		// 	 c1.getEngine() 호출코드와 String edu.kh.poly.ex1.model.vo.Car.getEngine() 메소드 코드연결
		// --> [정적 바인딩]
		
		System.out.println(c1.toString());
		// String edu.kh.poly.ex1.model.vo.Car.toString()
		// Car 참조변수 c1을 이용해서
		// Car 객체에 있는 오버리이딩 된 toString() 메소드를 호출
		...
		System.out.println("----------------------------------------");
		// 하지만 실행 해보면 자식(Spark)의 toString()이 호출되는 것을 확인가능!
		...
		for(int i=0; i<arr.length; i++) {
			...
			// 실행 후 : 각 개체의 오바라이딩된 toString()이 호출됨 - 동적 바인딩
			
		}
		
		// ** 동적 바인딩 장점**
		// - 업캐스팅 상태의 참조변수를 별도의 다운 캐스팅없이 자식의 오버라이딩된 메소드를 수행할 수 있다.
