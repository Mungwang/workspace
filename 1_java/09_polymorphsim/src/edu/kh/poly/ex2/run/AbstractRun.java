...
import edu.kh.poly.ex2.model.service.MGMCalculator;
...
//		service.ex2();
		...
		// Calculator cal = new MGMCalculator();   
		
		// 인터페이스 == 미완성 설계도 == 객체 생성 불가능
		// -> 추상클래스 처럼 참조 변수로드 사용가능
		
		// 코드의 큰 수정없이 객체생성코드만 바꾸면 새로운 클래스 코드를 수행할수 있다.
		...
		// 인터페이스의 특징
		// 1) 인터페이스를 부모 참초 변수로 사용하면
		// 	  다형성 업캐스팅이 적용되서 상속받은 모든 클래스를 자식 객체로 참조할 수 있다.
		
		//    -> 이를 이용하여 중요한 메인 코드의 수정을 최소화 할 수있다.
		...
		//	  -> 이를 이용하여 공동 작업(팀 프로젝트)에 필요한 개발환경을 조성할 수있다.
	}
}
