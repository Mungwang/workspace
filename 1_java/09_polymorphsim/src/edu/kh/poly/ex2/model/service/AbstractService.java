...
public class AbstractService {
	...
	public void ex1() {
		...
		// 클래스 : 객체의 속성, 기능을 정의한것 (일종의 설계도)
		// 추상클래스 : 미완성 메소드를 포함한 클래스 (미완성 설계도)
		// -> 미완성 설계도로는 객체를 만들 수 없다!! -> 오류 발생
		
		// 해결 방법 : Animal을 상속 받아 미완성부분을 구현한 클래스를 이용해 객체생성
		
		// * 추상클래스를 상속받은 자식 객체 생성하기
	...
	public void ex2() {
		...
		// Animal 부모 = person 자식객체에 주소값을참조한다 ( 다형성중 업캐스팅)
		
		arr[1] = new Fish("물고기","잡식");
		// Animal 부모 = Fish 자식객체에 주소값을참조한다 ( 다형성중 업캐스팅)
		...
		for(int i=0; i<arr.length; i++) {
			...
			// 프로그램 실행 시 참조하고 있는 자식 객체의 오버라이딩된 eat()메소드 수행 - 동적 바인딩
			// (부모 타입 참조 변수로 메소드를 호출 했지만 자식타입에 오버라이딩된 메소드가 수행된다)
			
			// ★★★★★업캐스팅 상태(부모 참조 = 자식 객체)에서 부모 메소드 호출시, 오버라이딩된 자식 메소드 수행★★★★★
