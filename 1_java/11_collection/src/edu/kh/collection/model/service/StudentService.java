...
public class StudentService {
	...
	//						-> 하지만 리스트는 크기가 늘었다 줄었다하기때문에 큰의미없음
	...
	public StudentService() {
		...
		studentList.add(new Student("정해인",23,"경기도 안산",'M',80) );
		studentList.add(new Student("박보영",21,"서울시 강남구",'F',90) );
		studentList.add(new Student("김민서",33,"충북 청주시",'F',100) );
		studentList.add(new Student("문광민",29,"서울시 중랑구",'M',90) );
	}
	...
	public void ex() {
		...
		// 실행 후 : 알고보니 0번째는 Student객체이고,toString()이 오버라이딩 되어있음
		//		  -> Student의 toString()이 수행된 == 동적 바인딩
		
		// Student의 이름만 얻어오기
		// Student 객체가 맞는지 확인하고 다운 캐스팅을해야
	...
	// 인터페이스 -> 모든 메소드가 public abstract(추상메소드)
	//         -> 미완성 메소드가 포함되어있다.
	//		   -> 미완성된 부분이 존재하는 클래스이기 떄문에 객체를 생성해도 미완성 객체가된다
	//         -> 객체생성불가
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				System.out.println("\nerror : 입력형식이 유효하지않습니다. 다시시도해주세요.");
				sc.nextLine(); // 입력 버퍼에 남아있는 잘못 입력된 문자열제거
				
				menuNum = -1; // 첫 반복시 잘못 입력하는 경우 menuNum이 0을 가지고있어 종료되는데
							  // 이를 방지하기 위해 임의값 -1 대입
			}
	...
	public String addStudent() throws InputMismatchException{
		...
		if(studentList.add(new Student(name, age, region, gender, score) )) {
			...
			// (반환형)             > 제네릭 <Student> 때문에 List 타입이 Student로 제한됌
			
			// add()는 무조건 true 반환하기 때문에 솔직히 실패하는경우는 없음
			// 대신, 예외가 발생해서 add()수행전 메소드가 종료될순 있음.
			return "성공";
	...
	public void searchName1() {
		...
		for(Student std : studentList) {
			if(input.equals(std.getName())) {
				System.out.println(std);
				flag = false; // 3번째에 있더라도 5번째까지 반복은한다.
			}
		}
