...
public class SetService {
	...
	// Set(집합)
	// - 순서를 유지하지 않음(==인덱스 없음)
	// - 중복을 허용하지 않는다. (+ null도 중복X, 1개만 저장가능)
	...
	public void ex1() {
		...
		//인터페이스
		Set<String> set = new HashSet<String>(); // <> 제네릭
		...
		// Set.add(String e) : 추가 (원래는 String 자리에 Object인데 <>제레닉으로 제한해뒀다)
		set.add("네이버");
		set.add("카카오");
		set.add("라인");
		set.add("쿠팡");
		...
		// Set은 순서가 없어서 저장된 객체 하나를 얻어올 수 있는 방법이없다!
		// -> 대신에 Set 전체의 데이터를 하나씩 반복적으로 얻어올 순있다.
	...
	public void ex2() {
		...
		System.out.println(mem1 == mem2); // 주소 비교
										  // 얇은 복사 경우가 아니라면 다 false
		
		// mem1과 mem2가 가지고 있는 필드 값이 같은지 비교
		if(mem1.getId().equals(mem2.getId()) ) { // 아이디가 같은경우
	...
	public void ex3() {
		...
		// hashCode() 오버라이딩 전
		// -> equals()가 오버라이딩 되어 있지만 중복제거가 되지않음 ㅠ.ㅠ
	...
	public void ex4() {
		...
		double num2 = Double.parseDouble("1.2345"); // 문자열 "1.23456"을 double 형식으로 변환
	...
	public void ex5() {
		...
		// Wrapper 클래스이 AutoBoxing / AutoUnboxing
	...
	public void lotto() {
		...
		// int로 타입 제한을 할 수 없다!
		// 왜? int 기본 자료형이기 때문에 객체만 저장하는 컬렉션에는 들어갈수없다.
		
		// -> 해결 방법 : Wrapper Class를 이용해서 기본 자료형을 객체로 포장한다.
		
		// Set<Integer> lotto = new HashSet<Integer>();
		// Set<Integer> lotto = new LinkedHashSet<Integer>(); // 입력된값이 차레대로나옴
		Set<Integer> lotto = new TreeSet<Integer>(); // 낮은값부터 순차적으로나옴

		// Integer는 equals(),hashCode() 오버라이딩 완료 상태
		
		while(lotto.size() < 6) {
			// lotto에 저장된 값이 개수가 6개 미만이면 반복 길이= size
