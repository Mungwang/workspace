...
public class ClsService /*(상속)extends Student*/ {
						//부모 Student를 상속 받음
	...
	public void ex1() {
		...
		// (default) class인 TestVo는 import 불가
	...
	public void ex2() {
		...
		// 정보 출력
		System.out.println(std1.schoolName); //public 이기때문에 직접 접근가능
		System.out.println(std1.getName());
		
		System.out.println(std2.schoolName); //public 이기때문에 직접 접근가능
	...
	// schoolName에 노란줄이 뜨는이유 -> 제대로 작성하지 않아서
	...
	public void ex4() {
		...
		//		  why? 같은 기본 생성자 User 객체를 생성했기때문에
		...
		User u3 = new User("test3", "pass333"); // 매개변수 생성자
							// 생성자 수행시 전달할 값을 작성 ★순서 꼭지켜야됨!★
		
		System.out.println(u3.getUserId());
		System.out.println(u3.getUserPw());
	}
	
	public void ex5(){ // 매개변수 생정자 예제
		
		User u1 = new User();// 기본생성자		
		User u2 = new User("user02","pass02"); // 매개변수 2개 생성자
		User u3 = new User("user03","pass03","청춘",25,'남'); // 매개변수 5개 생성자
		User u4 = new User("user04","pass04","청춘",28,'여'); // 매개변수 5개 생성자
		User u5 = new User("user05","pass05","청춘",21,'남'); // 매개변수 5개 생성자
