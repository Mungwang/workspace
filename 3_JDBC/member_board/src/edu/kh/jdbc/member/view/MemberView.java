...
public class MemberView { // 회원 관련 화면 입/출력
	...
	public void signUp() {
		...
		try {
			...
			while (true) { // 중복 아이디가 없을 경우 종료하는 if문 필요
				...
				memberId = sc.next();
				// 아이디 중복검사 (DB에 일치하는 아이디 있으면 "중복" -> 다시 아이디 입력 받기
				...
				if (result == 0) { // 중복 아니면 반복문 종료
					...
					System.out.println("[이미 사용중인 아이디입니다. 다시 입력 해주세요.");
				}
	...
	public Member login() {
		...
		try {
			...
			}else { // 로그인 실패(아이디 또는 비밀번호가 불일치 또는 탈퇴한 회원)
	...
	public void selectAll() {
		...
		try {
			...
			if(memberList.isEmpty()) { // 비어있음 == 조회결과없음
	...
	public void updateMyInfo(Member loginMember) {
		...
		try {
			// UPDATE == DML == 수행 성공한 결과 행의 개수를 반호나 == 정수형
			...
			if(result>0) { // 수정성공
			    ...
			    // -> 특징 : 복사된 주소를 참조하여 수정하면 원본 객체가 수정
	...
	public void updatePw(Member loginMember) {
		...
		try {
			...
			System.out.println("[비밀번호 변경중 예외발생]");
			e.printStackTrace();
		}
	}
	...
	public int secession(Member loginMember) {
		...
		// -> 복사본이 사라진다고 해도 원본(MainView의 loginMember)은 사라지지않는다
		// -> 로그아웃이 안된다.
