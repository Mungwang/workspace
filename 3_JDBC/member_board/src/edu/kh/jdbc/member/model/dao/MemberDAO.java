...
//											-> DB 접근할 수 있다 == SQL을 수행하고 결과를 반환 받을수있다.
...
public class MemberDAO {
	...
	private ResultSet rs = null; // 자동으로 null값이 들어가지만 그냥적어뒀다
	...
	// MemberDAO 기본생성자
	public MemberDAO() {
		// MemberDAo 객체생성시 member-sql.xml 파일의 내용을 읽어와
		// Properties 객체 생성
	...
	public int duplicateCheck(Connection conn, String memberId) throws Exception {
		...
		try {
			...
			// 6) 조회 결과를 한 행씩 접근하여 원하는 컬럼 값 받아오기
			// -> 아이디 중복 검사 SELECT 결과는 0 또는 1이라는 1행 결과가 무조건나옴
	...
	public int signUp(Connection conn, Member signUpMember) throws Exception {
		...
		try {
			...
			result = pstmt.executeUpdate(); // 성공한 행의 개수가 반환
		} finally {
			// 5) 사용한 JDBC 자원 반환(Connection 제외 Service 에서 닫아줄예정)
			close(pstmt);
