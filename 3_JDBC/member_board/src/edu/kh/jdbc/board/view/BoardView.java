...
public class BoardView {
	...
	private void insertBoard(int memberNo) {
		...
		try {
			...
			if(result>0) {
				...
				System.out.println("게시슬 작성실패");
			}
		} catch (Exception e) {
			System.out.println("<게시글 작성중 예외발생>");
			e.printStackTrace();
		}
	...
	private void selectOne(Member loginMember) {
		...
		try {
			...
			if (board != null) {
				...
				switch (menuNum) {
				case 0:
					System.out.println("n게시판 메뉴로 돌아갑니다...\n");
					break;
				case 1:
				...
				case 3:
					...
					if (reply == null) { // 같은 댓글 번호가 목록에 없는 경우
						...
						if (reply.getMemberNo() == loginMember.getMemberNo()) {
							...
							if (menuNum == 2)
								updateReply(inputNo); // 댓글수정
							else
								deleteReply(inputNo); // 댓글삭제
	...
	private void updateReply(int inputNo) {
		...
		try {
			...
			if (result > 0) {
				System.out.println(inputNo + "번 게시글이 수정되었습니다\n");
			} else {
				System.out.println("\n 댓글 수정 실패\n");
			}
	...
	private void insertReply(Member loginMember, int boardNo) {
		...
		// 회원번호, 게시글 번호, 댓글 내용 -> 하나의 Reply객체에 저장
		Reply reply = new Reply();
	...
	private void updateBoard(int boardNo) {
		...
		System.out.println("\n수정할 내용(종료 시 @exit 입력)\n");
		String boardContent = inputContent(); // 내용 입력메소드 호출 후 결과 받기
	 ...
	 * ex) String str = "abc";
	 * 
	 * str +="123"; // "123"이 저장된 String 객체 생성 후 // "abc"와 "123"이 합쳐진 String 객체가 추가로
	 * 별도 생성 // 그 후 "abc123" 객체의 주소를 str에 저장
	 * 
	 * ** String의 문제점 **
	 * 
	 * - String에 저장된 값을 바꾸거나 누적하려고 할 때마다 String 객체가 무분멸하게 생성됨 --> 메모ㅠ리 낭비(메모리 누수)
	 * 
	 * ** 해결방법 ** - StringBuffer / StringBuilder (가변성) 클래스를 자바에서 제공함
	 * 
	 * (StringBuffer / StringBuilder는 사용 방법은 똑같음) -> 차이점은 동기/비동기 차이밖에 없음
	 * 
	 */
	...
	private String inputContent() {
		...
		while (true) { // @exit가 입력될때 까지 무한히 문자열을 입력 받아
			// 하나의 변수에 누적 == 게시글 내용

			input = sc.nextLine();

			if (input.equals("@exit")) { // @exit가 입력된경우
				break;
			} else {
				// content += input + "\n"; // 줄바꾸며 누적
	...
	private void deleteBoard(int boardNo) {
		...
		while (true) {
			...
			if (ch == 'Y' || ch == 'N') { // Y 또는 N인 경우 추가입력X (반복종료)
				break;
			} else {
		...
		if (ch == 'Y') { // 삭제를 하려고 하는경우
			...
			if (input.equals(cap)) { // 입력받은 문자열과 보안 문자가 같을때
				// 삭제 Service 호출
