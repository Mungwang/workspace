...
-- [관리자 계정] member 계정 생성
CREATE USER mgm_member IDENTIFIED BY member1234;

-- [관리자 계정] CONNECT, RESOURCE + CREATE VIEW 권한 부여 + 객체 생성공간 할당
GRANT CONNECT, RESOURCE, CREATE VIEW TO mgm_member;
ALTER USER mgm_member DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- mgm_meber 계정 접속 방법추가
...
-- 각 테이블 PK 생성용 시퀀스 생성
CREATE SEQUENCE SEQ_MEMBER_NO INCREMENT BY 1; -- 1부터 1씩 증가, 반복X
CREATE SEQUENCE SEQ_BOARD_NO  INCREMENT BY 1;
CREATE SEQUENCE SEQ_REPLY_NO  INCREMENT BY 1;
