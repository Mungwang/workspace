...
<%-- 
	...
	pageEncoding="UTF-8" 	: 현재 문서를 해석할 때 UTF-8 문자인코딩을 이용해서 해석
	
	<% %> : 스크립틀릿(scriptlet) -> JSP에서 자바 코드를 작성할 수 있는 영역
	-> JSTL 라이브러리를 이용해서 태그 형식으로 변경
	
	
	<%= %> : 표현식(EXpression) -> 자바 코드의 값을 HTML 형식으로 포현(출력)
...
<% // 자바 코드 작성 영역 (scliptlet, 스크립틀릿)
	...
	// JSP에서도 요청 시 전달 받은 값(parameter)을 얻어올수 있다.
