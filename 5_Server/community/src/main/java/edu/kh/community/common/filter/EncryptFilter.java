...
// 암화를 적용 해야하는 요청 : 로그인, 회원가입, 비밀번호 변경, 회원탈퇴

// 필터가 적용될 url이 여러 개인 경우 : String 배열 초기화 형태 {}로 작성 / member에 관한게 여러개이면 /member/* 가능
...
public class EncryptFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// doFilter 메소드 매개변수는 부모타입인 SetvletRequest
		// -> 다운 캐스팅 필요
		
		HttpServletRequest req = (HttpServletRequest)request;
		
		// 파라미터를 다시 세팅하는 방법은 필터에서 불가능함
		// -> Setvlet의 Wrapper 클래스를 이용해서
		// 	  HttpServletRequest의 메소드를 오버라이딩 할 수 있다.
		// 	  -> 오버라이딩(재정의)를 통해서 비밀번호 암호화 진행
		
		// wrapper 객체를 생성해서 기존 HttpSerlvetRequeset 객체 역할을 대체한다.
