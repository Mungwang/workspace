...
public class BoardWriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(mode.equals("update")) {
				...
				req.setAttribute("detail", detail); // jsp에서 사용할 수있도록 req에 값 세팅
			}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			int maxSize = 1024 * 1024 * 100; // 업로드 최대 용량 (100MB) * 1024 = 1KB
			...
			// 	 Enumeration 형태로 반환 (Iterator)
			// 	 --> 해당 객체에 여러 값이 담겨 있고 이를 순서대로 얻어오는 방법을 제공
			//		 (보통 순서가 없는 모음(SET과 같은경우)에서 하나씩 꺼낼 때 사용
			...
			while(files.hasMoreElements()) { // 다음 요소가 있으면 true
				...
				String original = mpReq.getOriginalFileName(name); // 원봄 파일명
				
				// System.out.println("rename = " + rename);
				// System.out.println("original = " + original);
				
				if(rename != null) { // 업로드된 파일이 있을경우 ==
									 // 현재 files에서 얻어온 name 속성을 이용해
									 // 원본 또는 변경을 얻어왔을 때 그값이 null이 아닌경우
					
					// 이미지 정보를 담은 객체(BoardImage)를 생성
					BoardImage image = new BoardImage();
					
					image.setImageOriginal(original); // 원본명 (다운로드시 이용)
			...
			if(mode.equals("update")) { // 수정
				// 앞선 코드는 동일(업로드된 이미지 저장, imagesList 생성, 제목/내용 파라미터 동일)
