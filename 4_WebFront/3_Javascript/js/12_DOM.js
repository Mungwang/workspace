...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    // 부모 노드 마지막에 새로운 노드 추가(append : (마지막)에 덧붙인다)`
    ...
    // 4) 중간에 존재하는 자식 노드 탐색 : 부모요소.childNoes[인덱스]
...
document.getElementById("btn3-1").addEventListener("click",function(){
    ...
    const div = document.getElementById("div3-1"); // #div3-2 선택
...
document.getElementById("btn3-2").addEventListener("click",function(){
    ...
    // document.createrElement("태그명") : 해당 태그 요소를 생성하여 반환

    const child = document.createElement("div"); // div 생성 o, 화면 배치x

    
    if(count1 <=10){
        // 만들어진 div(child)에 내용 추가
        child.innerText = count1;
        count1++;
