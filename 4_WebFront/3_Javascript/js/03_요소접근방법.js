...
function accessName(){
    ...
    for(let i=0; i<hobbyList.length; i++){
        ...
        // .checked : 해당요소가 체크 되어있으면 true , 아니면 false 반환
    ...
    document.getElementById("name-div").innerHTML += "<br>선택된 갯수는 :" + count;
}
...
function readValue(){
    ...
    // 요소.scrollTop           : 요소 내부 현재 스크롤 위치 변환
...
function inputEnter(event){
    ...
    // console.log(envet.key); // 현재 눌러진 키를 반환

    if(event.key == "Enter"){ // 눌러진 key가 Enter
        readValue(); //함수호출
    }
}