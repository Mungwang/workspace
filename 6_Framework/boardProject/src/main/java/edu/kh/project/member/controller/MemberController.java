...
public class MemberController {
	...
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// 파라미터 전달 방법 3 : @ModelAttribute를 이용한방법
	...
	public String login(Member inputMember, Model model
		...
		if(loginMember !=null) { // 로그인 성공 시
			 ...
			 * 
			 * redirect된 페이지에서는 이전 요청이 유지되자않는다!!!
			 * -> 유지하고 싶으면 어쩔수 없이 Session을 이용
			 * 
			 * [Spring]
			 * 이런 상황을 해결하기 위한 객체
	...
	public String logout(SessionStatus status, HttpSession session) {
		...
		// 세션 무효화
		// Servlet -> HttpSession.invaildate()
		
		// Spring
		// 1) HttpSession을 이용한경우
		//  -> HttpSession.invalidate();
		
		
		// 2) Model + @SessionAttributes 이용할 경우
	...
	public String signUp() {
		...
		// -> ** ViewResovler가 prefix, suffix를 리턴값 앞,뒤에 붙임 ** 
