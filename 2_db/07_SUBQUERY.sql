...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE를 가지는 사원조회
...
NVL((SELECT EMP_NAME FROM EMPLOYEE E2 WHERE E1.MANAGER_ID = E2.EMP_ID),'없음' )관리저명
FROM EMPLOYEE E1;
...
--> SELECT 해석 순서를 고려하지 않아서 제대로 조회 되지않음
...
FROM EMPLOYEE
ORDER BY SALARY DESC;
--> 해결 1) 조회결과 (RESULT SET)을 가상의 테이블(==VIEW) 취급을 할 예정

--> 해결 2) 해결 1)의 조회결과를 FROM 절에 사용한 후 상위 5행만 조회하기
...
-- RANK() OVER : 동일한 순위 이후의 등수르 동일한 인원 수 만큼 건너뛰고 순위 계산
