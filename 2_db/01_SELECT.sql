...
-- EMPLOYEE 테이블에서 모든 사원은 사번,이름,전화번호 조회
...
-------------------------------------------------------
-- <컬럼 값 산술 연산>
-- 컬럼 값 : 테이블의 한 칸(== 한셀)에 작성된 값(DATA)

-- SELSELECT 작성 시 컬럼명에 산술 연산을 작성하면
-- 조회되는 결과 컬럼 값에 산술 연산이 반영된다.

-- EMPLOYEE 테이블에서 모든 사원의 사번,이름,급여 조회
...
-- SYSDATE : 시스템상의 현재 날짜
--          (년,월,일,시,분,초 단위까지 표현가능하지만,
--          디벨로퍼의 날짜 표기 방법이 년/월/일로 지정되어있는것)

-- DUAL(DUmmy tAbLe) : 가짜 테이블(임시테이블, 단순조회테이블)
...
*/
 ...
 -- DB에서의 리터럴 : 임의로 지정한값을 기존 테이블에 존재하는 값처럼 사용
 --> 리터럴 표기법 ''(홑따옴표)
...
-- (주의사항)
-- 1) DISTINCT는 SELLECT문에 딱 한번만 작성할 수 있다.
-- 2) DISTINCT는 SELECT문 가장 앞에 작성 되어야 한다.

-- EMPLOYEE 테이블에 지정된 직원들이 속해있는 부서 코드 종류 조회
...
-- EMPLOYEE 테이블에서 부서코드가 'D9'인 직원의 
-- 사번,이름,부서코드,전화번호 조회
SELECT EMP_ID,DEPT_CODE, EMP_NAME, DEPT_CODE, PHONE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
-- 논리 연산자 (AND,OR)
-- EMPLOYEE 테이블에서 급여가 200만 이상이고 부서 코드가 'D6'인 직원의
-- 이름, 급여, 부서코드를 조회
SELECT EMP_NAME,SALARY,DEPT_CODE 
FROM EMPLOYEE
WHERE SALARY >=2000000 AND DEPT_CODE = 'D6';

-- EMPLOYEE 테이블에서 급여가 300만 이상, 500만 이하인 직원의 
-- 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY >=3000000 AND SALARY <= 5000000;

-- EMPLOYEE 테이블에서 부서코드가 'D6' 또는 'D9'인 사원의
-- 사번, 이름, 부서코드 조회
...
-- ★★컬러명 BETWEEN A AND B : 컬럼 값이 A이상 B이하인 경우
--                          == 컬럼 값이 A미만, B초과인 경우★★

-- EMPLOYEE 테이블에서 급여가 200만 미만 ,500만 초과인 직원의
-- 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
...
/* BETWEEN을 이용한 날짜 비교 */
-- EMPLOYEE 테이블에서 입사일이 19990/01/01 ~ 1999/12/31인 (90년도입사자)
-- 직원의 사번, 이름, 입사일 조회
...
/**★★ '1990/01/01' 날짜를 문자열 형식으로 작성하게되면
...
-- 010으로 시작하지 "않는" 사람들 조회 ★★ NOT LIKE (LIKE 결과 부정)
SELECT EMP_ID,EMP_NAME,PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%';

-- EMPLOYEE 테이블에서 이메일에 _앞급자가 세글자인 사원 사번, 이름, 이메일 조회
SELECT EMP_ID, EMP_NAME , EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';

-- 문제점 : 와이들 카드 문자(_)와 패턴에 사용된 일반 문자(_)의 모양이 같아서 문제발생
-- 해결 방법 : ESCAPE OPTION을 이용하여 일반 문자로 처리할 '_', '%' 앞에 아무 특수문자나 붙임
SELECT EMP_ID, EMP_NAME , EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___$_%' ESCAPE '$'; --> & 뒤 한글자만(_)을 일반 문자 취급

-- 연습문제
-- 1. EMPLOYEE 테이블에서 이름 끝이 '연'으로 끝나는 사원의 이름 조회
-- 2. EMPLOYEE 테이블에서 전화번호 처음 3자리가 010이 아닌 사원의 이름,전화번호를 조회
-- 3. EMPLOYEE 테이블에서 메일주소 '_'의 앞이 4자이면서 DEFT_CODE가 D9 또는 D6이고
-- 고용일이 90/01/01 ~ 00/12/01 이고, 급여가 270만 이상인 사원의 전체를 조회
-- ★★ OR은 AND보다 우선순위가 낮아서 ( )로 감싸줘야한다.
SELECT EMP_NAME FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 부서코드가 D1 또는 D6 또는 D9인 사원의 사번,이름,부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
- SELECT문의 조회 결과(RESULT SET)을 정렬할 때 작성하는 구문
...
1 : FROM 테이블명 
2 : WHERE 조건식
4 : ORDER BY 컬럼명 | 별칭 | 컬럼순서  [정령 방식(오름/내림)] [NULLS FIRST | LAST]
...
FROM EMPLOYEE
-- WHERE SALARY >= 5000000 --> 해석 순서가 별칭 해석 전이기때문에 오류 발생
