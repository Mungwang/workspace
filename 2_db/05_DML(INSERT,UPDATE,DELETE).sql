...
VALUES(900,'문광민','951208-1234567','mum@naver.com','01045127845',
...
--(참고) INSERT시 VALUES 대신 서브쿼리 사용가능
CREATE TABLE EMP_01(
    EMP_ID NUMBER,
...
--서비쿼리(SQL에 내부에 포함된 SELECT문)를 이용한 삽입(VALUES 생략)
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'인 행의 DEPT_TITLE을 '전략기획팀'으로수정
...
-- * 조건절을 설정하지 않고 UPDATE 구문 실행시 모든 행의 컬럼값 변경
SELECT * FROM DEPARTMENT2;
...
--* 여러 칼럼을 한번에 수정할 시 콤마(,)로 컬럼을 구분하면됨
