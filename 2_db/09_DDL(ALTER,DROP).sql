...
-- DEPT_COPY 테이블의 LOCATION_ID 컬럼에 CHECK 제역조건 추가
...
-- NOT NULL은 기존 컬럼의 성질을 변겅하는 것으로 인식됨(MODIFY) 
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NOT NULL; -- NOT NULL은 MODIFY 사용
                                                  -- ★★★중요★★★

-------------------------------------------------------------------------------

-- DEPT_CODE에 추가한 제약조건 중 PK 빼고 모두 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_TITLE_U;
ALTER TABLE DEPT_COPY DROP CONSTRAINT LOCATION_ID_CHK;
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007474; -- NOT NULL

ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE CONSTRAINT SYS_C007474 NULL;
...
--            ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAUL '값'; (기본값 변경)

--> ** 데이터 타입 수정 시 컬럼에 저장된 데이터의 크기 미만으로는 변경할수 없다.

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP (삭제할컬럼명);
--            ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> ** 테이블에는 최소 1개 이상의 컬럼이 존재해야 되기 때문에 모든 컬럼 삭제X

-- 테이블이란? 행과 열로 이루어진 데이터베이스의 가장 기본적인 객체

-- (추가)
-- DEPT_COPY 테이블에 CNAME VARCHAR2(20) 컬럼추가
...
-- (추가)
-- DEPT_COPY 테이블에 LNAME VARCHAR2(30) 기본값 '한국'
ALTER TABLE DEPT_COPY 
...
-- (수정 에러 상황)
-- DEPT_TLTLE 컬럼의 데이터아입을 VARCHAR2(10)으로 변경
...
SELECT * FROM DEPT_COPY; --> 기본값을 변경했다고해서, 기존에 저장된 값이 변경되진않음.
...
ALTER TABLE DEPT_COPY DROP(DEPT_ID); -- 삭제실패
-- ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다

ROLLBACK; -- 트랜잭션(DML을 이용한 데이터 변경사항을 저장)을 삭제하고 마지막 커밋상태로 돌아감
...
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR(4); -- DDL 수행
ROLLBACK;
...
--> 제약조건까지 모두 삭제
-- * FK 제약조건으로 인해 삭제가 원래는 불가능 하지만, 제약조간을 없애버려서 FK 관계를 해제
...
-- 4. 컬럼, 제약조간,테이블 이름 변경(RENAME)

-- 테이블 복사
CREATE TABLE DEPT_COPY 
AS SELECT * FROM DEPARTMENT;

-- 복사한 테이블 PK 추가 (테이블관련 ALTER)
...
SELECT * FROM D_COPY;
SELECT * FROM DEPT_COPY;-- 이름이 변경되어 DEPT_COPY 테이블명으로 조회불가