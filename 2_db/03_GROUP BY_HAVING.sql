...
-- * GROUP BY절 : 같은 값들이 여러개 기록된 컬럼을 
--                같은 값들을 하나의 그룹으로 묶음
-- GROUP BY 컬럼명 | 함수식,..
-- 여러개의 값을 묶어서 하나로 처리할 목적으로 사용함
-- 그룹으로 묶은 값에 대해서 SELECT절에서 그룹 함수를 사용함

-- 그룹 함수는 단 한개의 결과 값만 산출하기 때문에 그룹이 여러 개일 경우 오류발생
...
-- EMPLOYEE 테이블에서
-- 부서코드, 부서 별 급여의 합계, 부서 별 급여의 평균(정수처리),인원수를 조회하여
-- 부서 코드 순으로 정렬
...
-- EMPLOYEE 테이블에서
-- 부서코드와 부서별 보너스를 받는 사원의 수를 조회하고 부서코드 순으로 정렬
...
-- EMPLOYEE 테이블에서
-- 성별과 성별 급여 평균(정수처리), 급여합계,인원 수 조회하고
-- 인원수로 내림차순 정렬
...
FROM EMPLOYEE
WHERE SUBSTR(HIRE_DATE,1,1)=2 --내가한거
    -- HIRE_DATE>='2000/01/01' 선생님
GROUP BY JOB_CODE;

-- * 여러 컬럼을 묶어서 그룹으로 지정가능 --> 그룹내 그룹이 가능하다!
-- *** GROUP BY 사용시 주의사항 ***
...
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE; -- 보너스 없는 것도 다샜다
...
-- 3.EMPLOYEE 테이블에서 
-- 부서별 70년대생의 급여(WHERE) / 평균이 300만 이상인 부서를 조회하여(HAVING)
-- 부서 코드 오름차순으로 정렬하세요
