...
SELECT LENGTH('HELLO WORLD') FROM DUAL; -- 띄어쓰기도 포함 11글자
...
SELECT '     K  H     ',TRIM('     K  H     ')FROM DUAL; -- 양쪽 공백제거

SELECT '---KH---', TRIM(TRAILING '-' FROM '---KH---') FROM DUAL;
-- BOTH 또는 생략 시 : 양쪽 '-' 기호제거
-- LEADING : 앞쪽만 제거
-- TRAILING : 뒤쪽만 제거
...
SELECT EMP_NAME, SALARY ,MOD(SALARY,1000000)나머지 FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 각 사원의 이름,입사년도,입사 월,입사 일조회
SELECT EMP_NAME 이름,
...
-- 0 : 숫자 한칸을 으미, 여러개 작성 시 오른쪽 정렬 + 빈칸 0 추가
...
-- 직원들의 입사일을 '2017년 12월 06일 (수)
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일" (DY)') 
FROM EMPLOYEE;
--> 년,월,일은 오라클에 등록된 날짜 표기 패턴이 아니라서 오류
--> 기존에 없던 패턴 추가시 ""(쌍따옴표)로 감싸줘서 문자열 그대로를 출력하게 된다

---------------------------------------------------------------------

/* 날짜로 변환 TO_DATE */
-- TO_DATE(문자열 데이터, [포맷]) : 문자형 데이터를 날짜로 변경
...
-- EMPLOYEE 테이블에서
-- 기존 보너스를 받던 사원의 보너스를 0.8로 변경
-- 보너스를 받지 못했던 사원의 보너스를 0.3으로 변경하여
-- 이름,기존보너스,변경된 보너스조회
SELECT EMP_NAME,NVL(BONUS,0),NVL2(BONUS,0.8,0.3)
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명,
FLOOR(ABS(HIRE_DATE-SYSDATE)) "근무일수1",FLOOR(ABS(SYSDATE-HIRE_DATE))"근무일수2"
FROM EMPLOYEE;
-- ABS = 절대값
...
/* 선택 함수 */
-- 여러 가지 경우에 따라 알맞은 결과를 선택할 수있음

-- DECODE
--(계산식 | 컬럼명, 조건값1, 선택값1, 조건값2, 선택값2....., 아무것도일치하지않을때)
...
FROM EMPLOYEE;
-- 그릅 함수는 여러 개를 동시 작성 가능한데
-- 이 때 결과는 실제 테이블 한 행이 아니고 각 그룹함수 별 독립된 결과

-- 사번이 200 아닌 사람들 중에서
-- EMPLOYEE 테이블에서 가장 높은 급여, 
-- 가장 늦은 입사일, 알파벳 순서가 늦은 이메일
...
-- COUNT(컬럼명) : NULL 제외한 실제 값이 기록된 행 개수를 리턴 함
