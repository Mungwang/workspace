...
데이터 딕셔너리는 사용자가 테이블을 생성하거나 사용자를 변경하는 드의
작업을 할 때 데이터베이스 서버에 의해 자동으로 갱신되는 테이블

- User_tables : 자신의 계정이 소유한 객체 등에 관한 정보를 조회할수 있는 딕셔너리 뷰
...
-- 테이블로 생성된 객체는 DROP 구문을 통해 제거 할 수있음

-- 1.테이블 생성하기
-- 테이블이란?
-- 행(ROW)과 열(COLUMM)으로 구성되는 가장 기본적인 데이터베이스 객체
-- 데이터 베이스 내에서 모든 테이터는 테이블을 통해서 저장된다.
...
-- MEMBER 테이블 생성
CREATE TABLE MEMBER(
    MEMBER_ID VARCHAR2(20),  -- 가변 길이 문자열20바이트 (영어,숫자만 작성시 20글자)
...
-- ★★ DEFAULT : 입력되는 값이 없거나 ' DEFAULT' 키워드 사용시에 기록되어질 값을 지정★★
...
-- 2) 컬럼에 주석 달기
-- [표현식]
-- COMMENT ON COLUM 테이블명,컬럼명 IS '주석내용';
...
--> NUMBER 타입 컬럼에 데이터 삽입 시 앞에 0이 있으면 사라지는 문제점이있음!!
...
/*
    사용자가 원하는 조건의 데이터만 유지하기 위해서 확정 컬럼에 설정하는 제약.
    데이터 무결성 보장을 목적으로 함.
    -> 데이터 무결성이란? 중복 데이터 최소화, NULL 초소화 지향(목표로한다)
...
-- ORA-01400: NULL을 ("MGM"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
     USER_NO NUMBER,
     -- USER_ID VARCHAR2 (20) UNIQUE -- 컬럼레벨 제약조건 설정 (제약조건명미지정)
     ...
     -- 테이블 레벨 제약조건 설정 부분
     --UNIQUE(USER_ID) -- 테이블 레벨 제약조건 설정(제약조건 미지정)
...
INSERT INTO USER_USED_UK
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(MGM.USER_ID_U)에 위배됩니다
--> 같은 아아디인 데이터가 이미 테이블에 있으므로 UNIQUE 제약 조건에 위배되어 오류발생
            --> UNIQUE
...
--  ORA-00001: 무결성 제약 조건(MGM.USER_ID_NAME_U)에 위배됩니다
...
-- 한개 컬럼에 설정할 수도 있고, 여러개의 컬럼을 묶어서 설정할 수있음(복합키)

SELECT * FROM EMPLOYEE WHERE EMP_ID = 200;

CREATE TABLE USER_USED_PK(
    
    USER_NO NUMBER /*CONSTRAINT USER_NO_PK*/ /*PRIMARY KEY*/, -- 컬럼레빌 PK지정
                   /*생략가능*/
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30),
    USER_NAME VARCHAR2(30),
...
--> 기본키 중복으로 오류
-- OBA-00001 : 무결성 제약 조건(MGM.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("MGM"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- 회원 번호와 아아디 둘다 중복 되었을 때만 제약조건 위배 에러 발생
-- ORA-00001: 무결성 제약 조건(MGM.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

-- PRIMARY KEY는 NULL이 들어갈 수없음
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
--> 복합키여도 PK이므로 NILL 절대안됨!!!!

----------------------------------------------------------------------

-- 4. FOREIGN KEY(외부키 / 외래키) 제약조건
--> 다른 테이블의 PK또는 UNIQUE 제약조건이 설정된 컬럼
...
CREATE TABLE USER_USED_FK(
  ...
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER, 
--CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼레벨 FK설정
...
-- 50이라는 값은 USER_GRADE 테이블에 GRADE_CODE 컬럼에서 제공하는 값이 아니므로 
-- 외래키 제약조건에 위배되어 오류 발생
-- ORA-02291: 무결성 제약조건(MGM.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
                --> 참초 무결성 위배
...
-- 어떤식으로 처리할 지에 대한 내용을 설정할 수있다.

SELECT * FROM USER_GRADE; -- 참조를 당하는 테이블 == 부모 테이블
SELECT * FROM USER_USED_EK; -- 참조를 하는 테이블 == 자식 테이블

DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(MGM.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

-- 1) ON DELETE RESTRICTED(삭제 제한)로 기본 지정되어있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함

-- GRADE_CODE WND 20은 외래키로 참조되고 있지 않으므로 삭제가 가능함.
COMMIT;
...
-- 부모 테이블인 USER_GRADES2에서 GRADE_CODE = 10 삭제
--> ON DELETE SE NULL 옵션이 설정되어 있어 오류없이 삭제됨
...
-- 부모키 삭제시 값을 사용하는 자식테이블의 컬럼에 해당하는 행 삭제됨
...
SELECT * FROM USER_GRADE3; -- 부모행삭제확인 (10없어짐)
SELECT * FROM USER_USED_FK3; -- 10을 참초하던 자식행 삭제확인(종속관계의행이 전부 사라짐)
--> ON DELETE CASCADE 옵션으로 인해 참조키를 사용한행이 삭제됨을 확인

---------------------------------------------------------------------------------

--5. CHECK 제약조건 : 컬럼에 기록되는 값에 조건 설정을 할 수있음
--   CHECK (컬럼명 비교연산자 비교값)
--   주의 : 비교값은 리터럴만 사용할 수있음, 변하는 값이나 함수 사용 못함
...
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록가능한데
-- '남자'라는 다른조건이 들어와서 에러발생
-- ORA-02290: 체크 제약조건(MGM.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
        ...
        PNO VARCHAR(20) /*CONSTRAINT UK_PNO UNIQUE*/ CONSTRAINT NN_PNO NOT NULL,
        GENDER VARCHAR(3)CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
        PHONE VARCHAR(20),
        ADDRESS VARCHAR(100),
        STATUS VARCHAR(3) /*CONSTRAINT CK_STATUS CHECK(STATUS IN('Y','N')) CONSTRAINT
                           NN_STATUS NOT NULL,*/
                         DEFAULT 'N' CONSTRAINT NN_STATUS NOT NULL,
...
COMMENT ON COLUMN USER_TEST.USER_ID IS '회원아아디';
...
-- 1) 테이블 전체 복사 ★ AS는 별칭아니고 AS뒤에 있는결과값으로 테이블을 만들겠다는뜻 ★
...
CONSTRAINT PK_DEPT_CODE FOREIGN KEY (DEPT_CODE) REFERENCES DEPARTMENT;
