...
--> 자주 사용하는하는데 매번 쓰기힘들다 ... -> VIEW 생성

-- 사번,이름,부서명,직급명 VIEW생성
CREATE VIEW V_EMP
...
-- (실행 시 관리자 계정으로 할 것)
GRANT CREATE VIEW TO MGM; -- Grant을(를) 성공했습니다.
--> 다시 이니셜 계정으로 접속 -> 뷰 생성 구문다시 실행
...
-- 삽입 확인
SELECT * FROM V_DCOPY2; --> VIEW에 'D0'가 삽인된걸 확인함
--> 가상의 테이블인 VIEW에 데이터 삽입이 가능한걸까?
...
-- ORA-01400: NULL을 ("MGM"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

--> VIEW 이용한 INSERT 시 원본 테이블에 삽입이 된다.
...
CREATE OR REPLACE VIEW V_DCOPY2
AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2
WITH READ ONLY; -- 읽기전용의 VIEW생성 (DML X)
...
-- CURRVAL 호출
SELECT SEQ_EMP_ID.CURRVAL FROM DUAL; -- NEXTVAL 호출 후 CURRVAL 호출 시 오류없음
-- 228
...
/*
   ...
   * 시퀀스를 잘못 다루어 번호가 공백이 있어서 다시 처음부터 시작하고 싶은경우
   --> 시퀀스 삭제 후 재생성
*/
...
-- EMPLOYEE_COPT4 테이블의 EMP_NAME 컬럼에 INDEX 생성
